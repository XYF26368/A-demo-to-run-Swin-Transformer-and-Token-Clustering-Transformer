...
# Record the relationship between the original tokens and merged tokens
# In the token upsampling process, use the recorded information to copy the merged token features to the corresponding upsampled tokens
# 不同stage输出之间的特征融合
# a: torch.Size([1, 196, 384]), b: torch.Size([1, 49, 728])
def token_downup(target_dict, source_dict):
    ...
    x_s = source_dict['x']
    idx_token_s = source_dict['idx_token']# 这部分相等 全部都是3136 = 56* 56
    idx_token_t = target_dict['idx_token']# 这部分相等 全部都是3136 = 56* 56
    T = target_dict['token_num'] # 196 784 3136
    B, S, C = x_s.shape # B: batch, S: 尺寸的平方, C: output channels torch.Size([1, 49, 728]) torch.Size([1, 196, 384]) torch.Size([1, 784, 192])
    N_init = idx_token_s.shape[1]# 这部分相等 全部都是3136 = 56* 56
    # print("N_init: ",N_init)
    ...
    if N_init < T * S:
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            #     print(B, S, C, T)
            #     print(A.shape)
            #     print(x_s.new_ones(B * S, 1).type(torch.float32).shape)
            weight = weight / all_weight[(idx_token_t).reshape(-1), 0]
            
            A = torch.sparse.FloatTensor(coor, weight, torch.Size([B * T, B * S])).type(torch.float32)
            # print("A")
            x_s = x_s.reshape(B * S, C).type(torch.float32) # 新加的
            # print("x_s")
            # sparse matmul
            # print("N_init < T * S: ",A.shape, x_s.shape, x_s.reshape(B * S, C).shape)
        ...
        A = A / (A.sum(dim=-1, keepdim=True) + 1e-6)
        # dense matmul
        # print("else: ", A.shape, x_s.shape)
        x_out = A @ x_s

    # print("before rehsape")
    x_out = x_out.reshape(B, T, C).type(x_s.dtype)
    # print("post reshape")
    # print("x_out shape before return: ", x_out.shape)
    return x_out # 实验表明在这一步的时候出错
...
def cluster_dpc_knn(token_dict, cluster_num, k=5, token_mask=None):
    ...
    with torch.no_grad():
        ...
        mask = density[:, None, :] > density[:, :, None]
        mask = mask.type_as(x)
        dist_max = dist_matrix.max(dim=-1, keepdim=True)[0].max(dim=-1, keepdim=True)[0]
        ...
        # 将tokens分配到最近的中心
        dist_matrix = dist_matrix.gather(1, index_down.unsqueeze(2).expand(-1, -1, N))
        idx_cluster = dist_matrix.argmin(dim=1)

        # 确保聚类中心合并到自身
        idx_batch = torch.arange(B, device=x.device).view(B, 1).expand(B, cluster_num)
        idx_tmp = torch.arange(cluster_num, device=x.device).view(1, cluster_num).expand(B, cluster_num)
        idx_cluster[idx_batch.reshape(-1), index_down.reshape(-1)] = idx_tmp.reshape(-1)

    return idx_cluster, cluster_num

# def cluster_dpc_knn(token_dict, eps=0.5, min_samples=5, token_mask=None):
#     """
#     使用DBSCAN算法对tokens进行聚类。
#     返回:
#         idx_cluster (Tensor[B, N]): 每个token的聚类索引。
#         cluster_num (int): 实际聚类数量。
#     参数:
#         token_dict (dict): token信息字典。
#         eps (float): 邻域半径参数。
#         min_samples (int): 簇内最少样本数。
#         token_mask (Tensor[B, N]): 表示token是否为空的掩码。非零值表示token有意义，零值表示token为空。
#     """
#     with torch.no_grad():
#         x = token_dict['x']
#         B, N, C = x.shape
...
#         idx_cluster = torch.full((B, N), -1, dtype=torch.int64, device=x.device)
#         cluster_num = 0

#         for b in range(B):
#             visited = torch.zeros(N, dtype=torch.bool, device=x.device)
#             for i in range(N):
#                 if visited[i] or (token_mask is not None and not token_mask[b, i]):
#                     continue
#                 neighbors = (dist_matrix[b, i] <= eps).nonzero(as_tuple=False).squeeze(1)
#                 if len(neighbors) < min_samples:
#                     visited[i] = True
#                     continue
#                 idx_cluster[b, neighbors] = cluster_num
#                 visited[neighbors] = True
#                 seeds = neighbors.tolist()
#                 while seeds:
#                     current_point = seeds.pop()
#                     current_neighbors = (dist_matrix[b, current_point] <= eps).nonzero(as_tuple=False).squeeze(1)
#                     if len(current_neighbors) >= min_samples:
#                         for neighbor in current_neighbors:
#                             if not visited[neighbor]:
#                                 visited[neighbor] = True
#                                 seeds.append(neighbor.item())
#                             if idx_cluster[b, neighbor] == -1:
#                                 idx_cluster[b, neighbor] = cluster_num
#                 cluster_num += 1
