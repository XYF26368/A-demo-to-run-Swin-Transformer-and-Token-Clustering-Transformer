...
# torch.Size([1, 3136, 64]) -> torch.Size([1, 128, 56, 56])
# torch.Size([1, 784, 128]) -> torch.Size([1, 128, 28, 28])
# torch.Size([1, 196, 320]) -> torch.Size([1, 128, 14, 14])
# torch.Size([1, 49, 512]) -> torch.Size([1, 128, 7, 7])
class MTA(BaseModule):
    def __init__(self,
                 in_channels=[64, 128, 320, 512],
                 out_channels= [96, 192, 384, 728], # 128 [96, 192, 384, 728] [128,128,128,128]
                 num_outs=1,
                 start_level=0,
                 end_level=-1,
                 num_heads=[2, 2, 2, 2],
        ...
        for i in range(self.start_level, self.end_level):
            merge_block = TCBlock(
                dim=out_channels[i], num_heads=num_heads[i], mlp_ratio=mlp_ratios[i],
    ...
    def forward(self, inputs):
        ...
        for i, lateral_conv in enumerate(self.lateral_convs):
            ...
            # print(f"tmp shape0: {tmp['x'].shape}")
            tmp['x'] = lateral_conv(tmp['x'].unsqueeze(2).permute(0, 3, 1, 2)).permute(0, 2, 3, 1).squeeze(2) # 执行卷积操作 通道数从输入通道到输出通道
            # print(f"tmp shape1: {tmp['x'].shape}")
            input_dicts.append(tmp)

        # merge from high level to low level
        # RuntimeError: The size of tensor a (384) must match the size of tensor b (728) at non-singleton dimension 2
        ...
        for i in range(len(input_dicts) - 2, -1, -1):# i取值从len(input_dicts) - 2到0
            ...
            #     print(input_dicts[i]['x'].shape, token_downup(input_dicts[i], input_dicts[i + 1]).narrow(2, 0, input_dicts[i]['x'].shape[2]).shape)
            #     input_dicts[i]['x'] = input_dicts[i]['x'] + token_downup(input_dicts[i], input_dicts[i + 1]).narrow(2, 0, input_dicts[i]['x'].shape[2])
            #     # print("seccessfully runned func token_downup")
            #     input_dicts[i] = self.merge_blocks[i](input_dicts[i])
            #     # print("successfully runned block merging")
            # except RuntimeError:
            #     # print(input_dicts[i])
            #     print("Runtime Error Happened !")
            #     # print(token_downup(input_dicts[i], input_dicts[i + 1]).narrow(2, 0, input_dicts[i]['x'].shape[2]).shape)
            #     print(f"{i}: ", input_dicts[i]['x'].shape, input_dicts[i]['idx_token'].shape, input_dicts[i]['token_num'], input_dicts[i]['agg_weight'].shape) # idx_token, x, token_num  .shape
            #     print(f"{i+1}: ", input_dicts[i+1]['x'].shape, input_dicts[i+1]['idx_token'].shape, input_dicts[i+1]['token_num'], input_dicts[i+1]['agg_weight'].shape)
            # print(f"{i}: ", input_dicts[i]['x'].shape, input_dicts[i]['idx_token'].shape, input_dicts[i]['token_num'], input_dicts[i]['agg_weight'].shape) # idx_token, x, token_num  .shape
            # print(f"{i+1}: ", input_dicts[i+1]['x'].shape, input_dicts[i+1]['idx_token'].shape, input_dicts[i+1]['token_num'], input_dicts[i+1]['agg_weight'].shape)
